...
static char const             *edge_cubie_string[] = {"UF", "UR", "UB", "UL",
                                               "DF", "DR", "DB", "DL",
                                               "FR", "FL", "BR", "BL",
                                               "FU", "RU", "BU", "LU",
                                               "FD", "RD", "BD", "LD",
...
static char const             *corner_cubie_string[] = {"UFR", "URB", "UBL", "ULF",
                                                 "DRF", "DFL", "DLB", "DBR",
                                                 "FRU", "RBU", "BLU", "LFU",
                                                 "RFD", "FLD", "LBD", "BRD",
                                                 "RUF", "BUR", "LUB", "FUL",
...
static char const            *a_twist_string[] = {"F ", "F2", "F'", "L ", "L2", "L'",
                                            "U ", "U2", "U'", "B ", "B2", "B'",
                                            "R ", "R2", "R'", "D ", "D2", "D'"};

static char const             *b_twist_string[] = {"F2", "L2", "U ", "U2", "U'",
                                            "B2", "R2", "D ", "D2", "D'"};
...
/* ========================================================================= */
   void  exit_w_error_message(char const *msg)
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  twist_on_sliceedge_init(unsigned short t_on_sl_table[A_N_TWIST][N_SLICEEDGE], unsigned short *t_on_sl_ptrs[A_N_TWIST])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  half_to_edgeperm_init(unsigned short h_to_ep_table[N_HALFEDGEPERM][N_HALFEDGEPERM], unsigned short *h_to_ep_ptrs[N_HALFEDGEPERM])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   int  unpacked_sliceedge_to_midperm(int sl_array_in[12], int mperm_array_out[4])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  a_corner_eflip_dist_init(unsigned char cf_dist_table[A_N_CORNER][A_N_EFLIP], unsigned char *cf_dist_ptrs[A_N_CORNER])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  a_corner_eloc_dist_init(unsigned char cl_dist_table[A_N_CORNER][A_N_ELOC], unsigned char *cl_dist_ptrs[A_N_CORNER])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  a_eflip_eloc_dist_init(unsigned char fl_dist_table[A_N_EFLIP][A_N_ELOC], unsigned char *fl_dist_ptrs[A_N_EFLIP])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  b_mid_cornerperm_dist_init(unsigned char mc_dist_table[B_N_MIDPERM][B_N_CORNERPERM], unsigned char *mc_dist_ptrs[B_N_MIDPERM])
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  b_mid_edgeperm_dist_init(unsigned char me_dist_table[B_N_MIDPERM][B_N_EDGEPERM], unsigned char *me_dist_ptrs[B_N_MIDPERM])
/* ------------------------------------------------------------------------- */
...
if ((flip % 2) != 0)
   {
   printf("Error: flip any edge cubie!\n");
   exit(1);
   status = 1;
   }
...
if ((twist % 3) != 0)
   {
   printf("Error: twist any corner cubie %sclockwise!\n",
           (twist == 1) ? "counter" : "");
		   exit(1);
   status = 1;
   }
...
if (edge_par != corner_par)
   {
   printf("Error: swap any two edge cubies or any two corner cubies!\n");
   exit(1);
   status = 1;
   }
...
/* ========================================================================= */
   int  cube_init(Cube *p_cube, char *line)
/* ------------------------------------------------------------------------- */
...
char *array[20];
	...
	strcpy (e_str[0] ,array[0]);
	strcpy (e_str[1] ,array[1]);
	strcpy (e_str[2] ,array[2]);
	strcpy (e_str[3] ,array[3]);
	strcpy (e_str[4] ,array[4]);
	strcpy (e_str[5] ,array[5]);
	strcpy (e_str[6] ,array[6]);
	strcpy (e_str[7] ,array[7]);
	strcpy (e_str[8] ,array[8]);
	strcpy (e_str[9] ,array[9]);
	strcpy (e_str[10] ,array[10]);
	strcpy (e_str[11] ,array[11]);
	strcpy (c_str[0] ,array[12]);
	strcpy (c_str[1] ,array[13]);
	strcpy (c_str[2] ,array[14]);
	strcpy (c_str[3] ,array[15]);
	strcpy (c_str[4] ,array[16]);
	strcpy (c_str[5] ,array[17]);
	strcpy (c_str[6] ,array[18]);
	strcpy (c_str[7] ,array[19]);		
...
for (ii = 0; ii < 12; ii++)
    ...
    if (jj == 24)
       {
       p_cube->edges[ii] = -1;
       printf("Error: improper edge cubie:  %s\n", e_str[ii]);
	   exit(1);
       stat = 1;
       }
    }
...
for (ii = 0; ii < 8; ii++)
    ...
    if (jj == 24)
       {
       p_cube->corners[ii] = -1;
       printf("Error: improper corner cubie:  %s\n", c_str[ii]);
	   exit(1);
...
/* ========================================================================= */
   void  output_solution(Stage1_node *node1_arr, Stage2_node *node2_arr)
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  stage2_search(Stage1_node *node1_arr, Stage2_node *node2_arr)
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  stage2_solve(Stage1_node  *node1_arr)
/* ------------------------------------------------------------------------- */
...
/* ========================================================================= */
   void  stage1_search(Stage1_node  *node_arr)
/* ------------------------------------------------------------------------- */
...
/* modified to only serach for solution to 15 hops */
for (ii = max_dist; ii <= 15; ii++)
    {
...
/* modified to take command line argument ad cube state as in format "UF,UR,UB,UL,LB,DB,DL,RD,FR,FL,DF,BR,LDF,RDB,DLB,RUF,LUB,DRF,FUL,BUR,"   */
/* ========================================================================= */
...
while (1)
      {
	  if(argc > 0)
		stat = cube_init(&cube_struc, argv[1]);
